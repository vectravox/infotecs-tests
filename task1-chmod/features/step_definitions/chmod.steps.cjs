...
Given('существует временный файл {string} с правами {int}', async function (filename, perms) {
...
Given('существует директория {string} с файлом {string}', async function (dirName, fileName) {
  const dir = await createTestDir();
...
Then('права файла {string} должны быть {int}', async function (filename, expectedPerms) {
  const filePath = path.join(testDir, filename);
  const perms = await getFilePermissions(filePath);
  assert.strictEqual(perms, expectedPerms.toString());
});

Then('права директории {string} должны быть {int}', async function (dirName, expectedPerms) {
...
Then('команда должна завершиться с ошибкой', function () {
  assert.notStrictEqual(lastExitCode, 0, 'Команда должна была завершиться с ошибкой');
});
